...
public class Decrypt implements WorkingWithData {
    @Override
    public void DoShift(String dataSource, int key, boolean needWriteInFile, String writeToFile, boolean needToReadFromFile, String fileToRead) throws IOException {
        ...
        for (int i = 0; i < dataSource.length() ; i++) {
            if (dataSource.charAt(i) >= 65 && dataSource.charAt(i) <= 90 && (dataSource.charAt(i) - key <= 65)) {
            ...
            } else if (dataSource.charAt(i) >= 97 && dataSource.charAt(i) <= 122 && (dataSource.charAt(i) - key <= 97)) {
